    ...
    async def collect_metrics(self) -> bool:
        ...
        try:
            ...
            metric_data = MetricData(
                ...
                signal_quality=(self.last_signal.signal_bars / 5) * 100,  # Convert to percentage
    ...
    async def check_connection(self) -> bool:
        ...
        if not self.connected:
            ...
            if self.reconnect_attempts < self.config.max_reconnect_attempts:
                logger.info(f"Attempting to reconnect modem {self.config.name} (attempt {self.reconnect_attempts + 1})")
                ...
                logger.error(f"Max reconnect attempts reached for modem {self.config.name}")
                return False
                
        # Check if auto-connect is enabled and modem is disconnected
        if self.config.auto_connect and self.last_status and not self.last_status.connected:
        ...
        if self.last_signal and self.last_signal.rssi < self.config.signal_threshold:
            logger.warning(f"Low signal on modem {self.config.name}: {self.last_signal.rssi} dBm")
        ...
        if self.config.data_limit_enabled and self.last_usage:
            ...
            if data_used_mb >= self.config.data_limit_mb:
                logger.warning(f"Data limit reached on modem {self.config.name}: {data_used_mb:.2f} MB")
                if self.last_status and self.last_status.connected:
                    logger.info(f"Disconnecting modem {self.config.name} due to data limit")
            ...
            try:
                ...
                for manager in self.modem_managers.values():
                    if manager.config.enabled:
                        ...
                        time_since_last = (datetime.now() - manager.last_collection).total_seconds()
        ...
        while self.running:
            try:
                ...
                if self.config_manager.load():
                    ...
                    new_uuids = set(m.uuid for m in self.config_manager.modems if m.enabled)
