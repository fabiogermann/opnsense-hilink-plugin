...
class ConfigManager:
        ...
        try:
            ...
            if modems is not None:
                for modem_elem in modems.findall('modem'):
                    modem = ModemConfig(
                        ...
                        password=self._decrypt_password(modem_elem.findtext('password', '')),
                        auto_connect=modem_elem.findtext('auto_connect', '1') == '1',
                        roaming_enabled=modem_elem.findtext('roaming_enabled', '0') == '1',
                        max_idle_time=int(modem_elem.findtext('max_idle_time', '0')),
                        network_mode=modem_elem.findtext('network_mode', 'auto'),
                        reconnect_interval=int(modem_elem.findtext('reconnect_interval', '60')),
                        max_reconnect_attempts=int(modem_elem.findtext('max_reconnect_attempts', '3')),
                        collect_interval=int(modem_elem.findtext('collect_interval', '30')),
                        signal_threshold=int(modem_elem.findtext('signal_threshold', '-90')),
                        data_limit_enabled=modem_elem.findtext('data_limit_enabled', '0') == '1',
                        data_limit_mb=int(modem_elem.findtext('data_limit_mb', '10240')),
            ...
            if alerts is not None:
                self.alerts = AlertConfig(
                    low_signal_threshold=int(alerts.findtext('low_signal_threshold', '-90')),
                    data_limit_enabled=alerts.findtext('data_limit_enabled', '0') == '1',
                    ...
                    smtp_password=self._decrypt_password(alerts.findtext('smtp_password', '')),
    ...
    def _save_to_xml(self) -> bool:
        ...
        try:
            ...
            ET.SubElement(general, 'enabled').text = '1' if self.general.enabled else '0'
            ET.SubElement(general, 'update_interval').text = str(self.general.update_interval)
            ET.SubElement(general, 'data_retention').text = str(self.general.data_retention)
            ET.SubElement(general, 'debug_logging').text = '1' if self.general.debug_logging else '0'
            ...
            for modem in self.modems:
                ...
                ET.SubElement(modem_elem, 'enabled').text = '1' if modem.enabled else '0'
                ET.SubElement(modem_elem, 'ip_address').text = modem.ip_address
                ET.SubElement(modem_elem, 'username').text = modem.username
                ET.SubElement(modem_elem, 'password').text = self._encrypt_password(modem.password)
                ET.SubElement(modem_elem, 'auto_connect').text = '1' if modem.auto_connect else '0'
                ET.SubElement(modem_elem, 'roaming_enabled').text = '1' if modem.roaming_enabled else '0'
                ET.SubElement(modem_elem, 'max_idle_time').text = str(modem.max_idle_time)
                ET.SubElement(modem_elem, 'network_mode').text = modem.network_mode
                ET.SubElement(modem_elem, 'reconnect_interval').text = str(modem.reconnect_interval)
                ET.SubElement(modem_elem, 'max_reconnect_attempts').text = str(modem.max_reconnect_attempts)
                ET.SubElement(modem_elem, 'collect_interval').text = str(modem.collect_interval)
                ET.SubElement(modem_elem, 'signal_threshold').text = str(modem.signal_threshold)
                ET.SubElement(modem_elem, 'data_limit_enabled').text = '1' if modem.data_limit_enabled else '0'
                ET.SubElement(modem_elem, 'data_limit_mb').text = str(modem.data_limit_mb)
                ET.SubElement(modem_elem, 'alert_email').text = modem.alert_email
                
            # Save alerts
            alerts = ET.SubElement(hilink, 'alerts')
            ET.SubElement(alerts, 'low_signal_threshold').text = str(self.alerts.low_signal_threshold)
            ET.SubElement(alerts, 'data_limit_enabled').text = '1' if self.alerts.data_limit_enabled else '0'
            ET.SubElement(alerts, 'data_limit_mb').text = str(self.alerts.data_limit_mb)
            ET.SubElement(alerts, 'email_alerts').text = '1' if self.alerts.email_alerts else '0'
            ...
            ET.SubElement(alerts, 'smtp_password').text = self._encrypt_password(self.alerts.smtp_password)
            ET.SubElement(alerts, 'smtp_use_tls').text = '1' if self.alerts.smtp_use_tls else '0'
    ...
    def validate(self) -> List[str]:
        ...
        for modem in self.modems:
            ...
            try:
                ...
                errors.append(f"Invalid IP address for modem {modem.name}: {modem.ip_address}")
                
            # Validate network mode
            valid_modes = ['auto', '4g_preferred', '3g_preferred', '4g_only', '3g_only']
            if modem.network_mode not in valid_modes:
                errors.append(f"Invalid network mode for modem {modem.name}: {modem.network_mode}")
                
            # Validate signal threshold
            if modem.signal_threshold < -120 or modem.signal_threshold > -50:
                errors.append(f"Signal threshold for modem {modem.name} must be between -120 and -50 dBm")
