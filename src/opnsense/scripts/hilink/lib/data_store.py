...
class DataStore:
    ...
    def fetch(self, 
              ...
              resolution: Optional[int] = None) -> Optional[Dict[str, Any]]:
        """
        Fetch data from RRD
        ...
        try:
            ...
            result = rrdtool.fetch(
                ...
                '--end', str(end_time),
                '--resolution', str(resolution) if resolution else '30'
            )
            ...
            for row in data:
                ...
                for i, name in enumerate(ds_names):
                    ...
                    response['metrics'][name].append(value if value is not None else None)
                current_time += step
            ...
            while last_idx >= -len(data['timestamps']):
                ...
                if metrics['signal_strength'][last_idx] is not None:
                    return MetricData(
                        ...
                        connection_state=int(metrics['connection_state'][last_idx] or 0),
    ...
    def get_statistics(self,
        ...
        try:
            ...
            connection_values = [v for v in data['metrics']['connection_state'] if v is not None]
            if connection_values:
                uptime_percentage = (sum(connection_values) / len(connection_values)) * 100
            ...
            if rx_values and tx_values:
                ...
                stats['total_data_rx'] = int(rx_values[-1] - rx_values[0]) if len(rx_values) > 1 else 0
                stats['total_data_tx'] = int(tx_values[-1] - tx_values[0]) if len(tx_values) > 1 else 0
    ...
    def generate_graph(self,
        ...
        try:
            ...
            graph_args = [
                ...
                '--watermark', f'Generated {datetime.now().strftime("%Y-%m-%d %H:%M")}',
